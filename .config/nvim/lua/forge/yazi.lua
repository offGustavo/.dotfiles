...
local PATH_SELECTED_FILES = PATH_CACHE .. "/yazi_selected_files"
...
function M.setup(user_opts)
	...
	if opts.enable_cmds then
		...
		vim.keymap.set("n", "-", ":Yazi<CR>")
    vim.keymap.set("n", "<leader>fd", function ()
      require("forge.yazi").open()
    end, { desc = "Open Yazi File Manager"})
		vim.cmd(string.format('command! YaziSplit lua require("forge.yazi").open(nil, "%s")', M.OPEN_MODE.split))
		vim.cmd(string.format('command! YaziVsplit lua require("forge.yazi").open(nil, "%s")', M.OPEN_MODE.vsplit))
		vim.cmd(string.format('command! YaziTabedit lua require("forge.yazi").open(nil, "%s")', M.OPEN_MODE.tabedit))
	end
end
